...
typedef struct {
    UINT Version;                      /* RSAEuro Version */
    UINT flags;                        /* Version Flags */
    BYTE ManufacturerID[32];           /* Toolkit ID */
    UINT Algorithms;                   /* Algorithms Supported */
} RSAEUROINFO;

/* Random structure. */

typedef struct {
  UINT bytesNeeded;                    /* seed bytes required */
  BYTE state[16];                      /* state of object */
  UINT outputAvailable;                /* number byte available */
  BYTE output[16];                     /* output bytes */
} R_RANDOM_STRUCT;
...
typedef struct {
  UINT bits;                           /* length in bits of modulus */
  BYTE modulus[MAX_RSA_MODULUS_LEN];   /* modulus */
  BYTE publicExponent[MAX_RSA_MODULUS_LEN];     /* public exponent */
...
typedef struct {
  ...
  BYTE buffer[8];                      /* data buffer */
  UINT bufferLen;                      /* buffer length */
} R_ENVELOPE_CTX;

/* Random Structures Routines. */

int R_RandomInit PROTO_LIST ((R_RANDOM_STRUCT *));
int R_RandomUpdate PROTO_LIST ((R_RANDOM_STRUCT *, BYTE *, UINT));
int R_GetRandomBytesNeeded PROTO_LIST ((UINT *, R_RANDOM_STRUCT *));
...
int R_GenerateBytes(BYTE *block, UINT len,
	R_RANDOM_STRUCT *random);

/* Cryptographic procedures. */

int R_DigestInit PROTO_LIST ((R_DIGEST_CTX *, int));
int R_DigestUpdate PROTO_LIST ((R_DIGEST_CTX *, BYTE *,
	UINT));
int R_DigestFinal PROTO_LIST ((R_DIGEST_CTX *, BYTE *,
	UINT *));
int R_SignInit PROTO_LIST ((R_SIGNATURE_CTX *, int));
int R_SignUpdate PROTO_LIST ((R_SIGNATURE_CTX *, BYTE *,
	UINT));
int R_SignFinal PROTO_LIST ((R_SIGNATURE_CTX *, BYTE *,
	UINT *, R_RSA_PRIVATE_KEY *));
int R_VerifyInit PROTO_LIST ((R_SIGNATURE_CTX *, int));
int R_VerifyUpdate PROTO_LIST ((R_SIGNATURE_CTX *, BYTE *,
	UINT));
int R_VerifyFinal PROTO_LIST ((R_SIGNATURE_CTX *, BYTE *,
	UINT, R_RSA_PUBLIC_KEY *));
int R_SealInit PROTO_LIST ((R_ENVELOPE_CTX *, BYTE **, UINT *, BYTE [8],
	UINT, R_RSA_PUBLIC_KEY **, int, R_RANDOM_STRUCT *));
int R_SealUpdate PROTO_LIST ((R_ENVELOPE_CTX *, BYTE *,
	UINT *, BYTE *, UINT));
int R_SealFinal PROTO_LIST ((R_ENVELOPE_CTX *, BYTE *,
	UINT *));
int R_OpenInit PROTO_LIST ((R_ENVELOPE_CTX *, int, BYTE *,
	UINT, BYTE [8], R_RSA_PRIVATE_KEY *));
int R_OpenUpdate PROTO_LIST ((R_ENVELOPE_CTX *, BYTE *,
	UINT *, BYTE *, UINT));
int R_OpenFinal PROTO_LIST ((R_ENVELOPE_CTX *, BYTE *,
	UINT *));

/* Cryptographic enhancements. */

int R_SignPEMBlock PROTO_LIST ((BYTE *, UINT *, BYTE *, UINT *,
	BYTE *, UINT, int, int, R_RSA_PRIVATE_KEY *));
int R_SignBlock PROTO_LIST ((BYTE *, UINT *, BYTE *, UINT, int,
	R_RSA_PRIVATE_KEY *));
int R_VerifyPEMSignature PROTO_LIST ((BYTE *, UINT *, BYTE *, UINT,
	BYTE *, UINT, int, int, R_RSA_PUBLIC_KEY *));
int R_VerifyBlockSignature PROTO_LIST ((BYTE *, UINT,
	BYTE *, UINT, int, R_RSA_PUBLIC_KEY *));
int R_SealPEMBlock PROTO_LIST ((BYTE *, UINT *, BYTE *, UINT *,
	BYTE *, UINT *, BYTE [8], BYTE *,
	UINT, int, R_RSA_PUBLIC_KEY *, R_RSA_PRIVATE_KEY *,
	R_RANDOM_STRUCT *));
int R_OpenPEMBlock PROTO_LIST ((BYTE *, UINT *, BYTE *, UINT,
	BYTE *, UINT, BYTE *, UINT,
	BYTE [8], int, R_RSA_PRIVATE_KEY *, R_RSA_PUBLIC_KEY *));
int R_DigestBlock PROTO_LIST ((BYTE *, UINT *,
	BYTE *, UINT, int));

int R_DecryptOpenPEMBlock PROTO_LIST ((R_ENVELOPE_CTX *, BYTE *,
	UINT *, BYTE *, UINT));
int R_EncryptOpenPEMBlock PROTO_LIST ((R_ENVELOPE_CTX *, BYTE *,
	UINT *, BYTE *, UINT));

/* Printable ASCII encoding and decoding. */

int R_EncodePEMBlock PROTO_LIST ((BYTE *, UINT *,
	BYTE *, UINT));
int R_DecodePEMBlock PROTO_LIST ((BYTE *, UINT *,
	BYTE *, UINT));
...
int R_GenerateDHParams PROTO_LIST ((R_DH_PARAMS *, UINT,
	UINT, R_RANDOM_STRUCT *));
int R_SetupDHAgreement PROTO_LIST ((BYTE *, BYTE *,
	UINT, R_DH_PARAMS *, R_RANDOM_STRUCT *));
int R_ComputeDHAgreedKey PROTO_LIST ((BYTE *, BYTE *,
	BYTE *, UINT, R_DH_PARAMS *));
...
#else
void R_memset PROTO_LIST ((POINTER, int, UINT));
void R_memcpy PROTO_LIST ((POINTER, POINTER, UINT));
int R_memcmp PROTO_LIST ((POINTER, POINTER, UINT));
#endif
