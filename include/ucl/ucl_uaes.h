 ...
 *
 * in case the user does not want to permit the UCL to access directly the
 * hardware, the interface to USIP&reg; must be passed to the UCL with the 
 * function #ucl_uaes_init.
 *
 * @n
 ...
 * 
 *  @ingroup UCL_UAES */
typedef struct usip_aes {
	int (*setkey)  (const u8 *key);
	int (*dkeygen) (void);
	int (*decrypt) (u8 *dataOut, u8 *dataIn);
	int (*encrypt) (u8 *dataOut, u8 *dataIn);	
} usip_aes_t;
 ...
 * @retval #UCL_INVALID_INPUT		@p key is #NULL and @p option is not 
 * @retval #UCL_INVALID_ARG			@p option is not one of the possbile values
 * @retval #UCL_NO_UAES_INTERFACE	No interface to the USIP&reg; AES
 * 
 * @ingroup UCL_UAES
 */
 ...
 * 
 * @retval #UCL_OK          			No error occurred
 * @retval #UCL_INVALID_INPUT		@p dataIn is #NULL
 * @retval #UCL_INVALID_OUTPUT		@p dataOut is #NULL
 * @retval #UCL_NO_UAES_INTERFACE	No interface to the USIP&reg; AES
 * 
 * @ingroup UCL_UAES
 */
